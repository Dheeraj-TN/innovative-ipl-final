...
function ProjectCard({ project }) {
  ...
  const [link, setLink] = useState(project.videos[0]);

  const rightMoveImg = () => {
    const curSlide =
      imgCurIndex === project.images.length - 1 ? 0 : imgCurIndex + 1;
    setImgCurIndex(curSlide);
  };
  const leftMoveImg = () => {
    const curSlide =
      imgCurIndex === 0 ? project.images.length - 1 : imgCurIndex - 1;
    setImgCurIndex(curSlide);
  };

  const rightMoveVdo = () => {
    const curSlide =
      vdoCurIndex === project.videos.length - 1 ? 0 : vdoCurIndex + 1;
    setVdoCurIndex(curSlide);
    setLink(project.videos[curSlide]);
  };
  const leftMoveVdo = () => {
    const curSlide =
      vdoCurIndex === 0 ? project.videos.length - 1 : vdoCurIndex - 1;
    setVdoCurIndex(curSlide);
    setLink(project.videos[curSlide]);
  };
    ...
    <div className="project_card_container">
      ...
      <div className="project_card_body">
        <div className="img_video_container">
          <div
            className="project_card_body_img_container"
            style={{ backgroundImage: `url(${project.images[imgCurIndex]})` }}
          >
            <b
              style={{ height: "350px", display: "flex", alignItems: "center" }}
            >
              <i
